...
public class WeatherController {
    ...
    @PostMapping("/takeByAddress")
    public String takeByAddress(String resolvedAddress, Model model) {
        try {
            List<WeatherDataCity> weatherDataList = repo.findAllByResolvedAddress(resolvedAddress);
            weatherDataList = repo.findAllByResolvedAddress(resolvedAddress);
            model.addAttribute("weatherDataLists", weatherDataList);
            // Récupérer la première journée de météo
            WeatherDataDay firstWeatherDay = weatherDataList.get(0).getWeatherDay().get(0);
